...
def main(): # 将主要逻辑封装在一个函数中
    ...
    results = model.train(
        ...
        # 训练参数
        epochs=300,        # 训练历元总数,每个历元代表对整个数据集进行一次完整的训练
        ...
        lr0=0.001,         # 初始学习率
        weight_decay=0.055, # L2正则化，对大权重进行惩罚，防止过拟合
        ...
        copy_paste=0.4,  # 小目标增强
        erasing=0.4,     #随机擦除，模拟小面积的完全遮挡
        mixup=0.1,       #混叠增强，将两张图片混合，能提升模型的泛化能力
