...
### **项目任务分解与时间规划 (To-Do List) - 更新版 (截至第19周)**

**项目核心：** 遮挡条件下遥感图像中的飞机目标检测 (多样化遮挡模拟, 干扰物检测, DOTA数据集, OBB回归, 高频特征辅助定位)

**团队规模：** 5人 (组长进行协调和进度管理)

---

#### **第一阶段：深入调研、方案修订与环境准备 (第12周 - 第13周初)**

*   **任务1：深入理解项目需求与关键技术调研 (全体成员)**
    *   [x] 仔细阅读课程设计任务书，明确目标、任务描述、数据说明和考核标准。(相关指南请参考：[Git、GitHub操作与团队协作指南](./collab_guide.md))
    *   [ ] **文献阅读深化**:
        *   完成老师提供的三篇核心参考文献的精读与分析。
        *   针对新增技术点（多样化遮挡模拟方法、干扰物检测策略、高频特征注意力机制、关键点回归及其在目标检测中的应用、几何约束损失函数等）进行专题文献调研。
    *   [ ] 小组讨论，统一对DOTA数据集特性、Faster R-CNN原理、多样化遮挡模拟方式、干扰物检测思路、高频特征利用方案、倾斜框检测思路的理解。
*   **任务2：数据准备与技术方案详细修订 (全体成员，成员A&B主导数据，全体参与方案)**
    *   [ ] **数据方面 (成员A&B)**:
        *   下载DOTA数据集，筛选飞机子集，熟悉数据格式与OBB标注。
        *   研究DOTA的OBB四个顶点的含义，规划如何将其作为关键点。
    *   [ ] **技术方案修订与细化 (全体)**:
        *   讨论并确定多样化遮挡模拟的具体实现方法和参数范围（高斯、不规则形状、透明度、纹理等）。
        *   确定干扰物类别的定义和标注策略。
        *   详细讨论高频特征注意力模块的选型与集成位置。
        *   设计关键点（OBB顶点）相对位置的参数化表示方法。
        *   初步构思如何利用回归的关键点优化检测框（联合损失或后处理）。
    *   [ ] **高层设计文档更新 (全体)**: 更新技术路线文档，勾勒系统整体技术方案和主要模块划分，明确新增模块的接口。
*   **任务3：制定详细工作计划与开发环境加固 (组长协调，全体确认；成员D或E主导环境)**
    *   [ ] 根据新的截止日期（19周）和技术方案，制定详细到周的任务分解、责任人及交付物。
    *   [ ] **开发环境 (成员D或E)**:
        *   搭建深度学习框架 (PyTorch)，安装OpenCV, NumPy等必要库。
        *   确认Git仓库已建立并可用，所有成员熟悉协作流程。
        *   （新增）调研并准备可能用到的注意力模块库或关键点处理相关库。

#### **第二阶段：详细设计与核心模块并行实现 (第13周中 - 第14周)**

*   **任务4：算法与系统详细设计及文档化 (分工合作，全体评审)**
    *   [ ] **多样化遮挡生成模块设计 (成员C)**: 详细设计各类遮挡（高斯、不规则形状、透明度、纹理、组合等）的生成算法及参数化方案。
    *   [ ] **干扰物特性与标注整合设计 (成员A)**: 详细设计如何将干扰物及其标签整合进数据预处理流程。
    *   [ ] **检测算法详细设计 (成员D&E，核心算法主力)**:
        *   确定Faster R-CNN的具体网络结构（骨干网络选择ResNet+FPN）。
        *   设计注意力机制模块的具体集成方案。
        *   详细设计OBB回归分支（8参数）。
        *   详细设计关键点相对位置回归分支（4个顶点的8参数相对OBB的归一化坐标）。
        *   设计分类分支以包含飞机、背景及干扰物。
        *   初步设计多任务损失函数（包含OBB回归、关键点回归、分类，以及探索性的几何一致性损失）。
    *   [ ] **实验方案详细设计 (成员B)**: 设计如何评估不同遮挡类型、干扰物检测、以及高频特征利用策略对检测结果影响的实验流程。
    *   [ ] **系统架构确认 (全体)**: 绘制更新的系统总体流程图，明确各模块接口。
    *   [ ] **文档化**: 撰写核心算法设计文档或详细的报告技术章节初稿。
*   **任务5：核心模块编码启动 (分工合作)**
    *   [ ] **数据预处理与增强模块 (成员A)**:
        *   DOTA飞机数据加载、解析、图像分块、OBB标注到图像块的映射。
        *   实现OBB顶点到归一化相对关键点坐标的转换。
        *   实现旋转等几何增强，确保关键点坐标同步变换。
    *   [ ] **多样化遮挡生成与干扰物注入模块 (成员C)**:
        *   编码实现各类遮挡生成函数。
        *   编码实现将遮挡物按参数应用于图像，并生成干扰物标签的函数。
    *   [ ] **飞机目标检测模型骨架与核心回归头 (成员D&E)**:
        *   搭建基础网络模型结构（ResNet+FPN）。
        *   实现OBB预测的头部结构。
        *   实现关键点相对位置预测的头部结构。
        *   实现包含干扰物检测的多类别分类头。
        *   初步集成注意力模块。
    *   [ ] **基础损失函数实现 (成员D或E)**: 实现OBB回归损失、关键点回归损失、多类别分类损失。

#### **第三阶段：模块完成、集成与初步测试 (第15周)**

*   **任务6：完成核心模块编码与单元测试 (各模块负责人)**
    *   [ ] 完成所有分配模块的编码工作。
    *   [ ] 对各自模块进行充分的单元测试，确保功能正确性与鲁棒性。
*   **任务7：系统集成与初步联调 (全体成员)**
    *   [ ] 将数据预处理（含遮挡与干扰物生成、关键点处理）、检测模型（含注意力、多头回归与分类）、损失函数等模块集成为一个初步可运行的训练和推理流程。
    *   [ ] 解决集成过程中出现的接口和逻辑问题。
    *   [ ] 使用少量样本进行冒烟测试，确保流程能跑通。
*   **任务8：构建多样化遮挡测试数据集 (成员A&C)**
    *   [ ] 利用多样化遮挡模块，在原始DOTA飞机数据上生成具有不同遮挡类型、程度、覆盖不同目标尺度的测试样本集（包含飞机和对应的干扰物标签）。
    *   [ ] 确保测试集的多样性和代表性，并记录好每个样本的遮挡参数和干扰物信息。

#### **第四阶段：模型训练、深度实验分析与代码完善 (第16周 - 第17周初)**

*   **任务9：模型训练与调优 (成员D&E主导，其他成员协助分析)**
    *   [ ] 在无遮挡原始DOTA飞机数据子集上进行模型预训练或基线训练。
    *   [ ] 利用生成的含多样化遮挡及干扰物的数据集进行模型的训练或微调。
    *   [ ] 重点调试与优化：
        *   注意力模块的有效性。
        *   OBB回归与关键点相对位置回归的协同。
        *   多任务损失函数的权重平衡。
        *   （若采用）几何一致性损失的引入与调试。
    *   [ ] 调整学习率、优化器等超参数，优化模型在OBB检测、干扰物检测及关键点预测上的性能 (mAP等)。
*   **任务10：综合实验与结果分析 (全体成员，各有侧重)**
    *   [ ] **实验执行 (成员B&C)**: 在构建的多样化遮挡测试集上运行训练好的模型，系统记录检测结果 (飞机OBB、干扰物框、关键点预测、置信度等)。
    *   [ ] **遮挡影响分析 (成员A)**: 分析不同遮挡类型和程度对飞机检测准确率、召回率、OBB精度、关键点定位精度的影响。
    *   [ ] **干扰物检测分析 (成员C)**: 评估模型对干扰物的检测与分类性能，及其对飞机检测的潜在影响。
    *   [ ] **高频特征与关键点作用分析 (成员D&E)**:
        *   评估注意力机制是否提升了对高频区域的关注。
        *   分析关键点回归的准确性。
        *   若实现了基于关键点的检测框优化，量化其对OBB定位精度的提升，尤其是在遮挡情况下。
    *   [ ] **尺度影响分析 (成员B)**: 分析目标尺度对各方面检测结果的影响。
    *   [ ] **结果汇总与可视化 (全体)**: 整理实验数据，制作图表进行清晰展示。
    *   [ ] 记录实验过程中的关键发现、问题和解决方法。
*   **任务11：代码整理与注释 (全体成员)**
    *   [ ] 确保最终代码结构清晰、规范，添加必要的JSDoc注释，保证注释比例。

#### **第五阶段：报告撰写与成果总结 (第17周中 - 第18周)**

*   **任务12：完成课程设计报告终稿 (分工合作，组长统稿)**
    *   [ ] **封面、摘要、目录 (组长)**
    *   [ ] **第一章 课题概述 (成员A)** - 更新研究现状，明确项目意义，突出遮挡和高频特征利用的挑战。
    *   [ ] **第二章 算法分析 (成员B&C)** - 详细描述设计的遮挡模拟算法（多样化）、干扰物检测思路、倾斜框飞机检测算法（含注意力机制、关键点回归、可能的几何约束）。
    *   [ ] **第三章 试验系统设计 (成员D&E)** - 详细描述系统架构、程序流程图、各模块功能和核心代码设计思路（重点突出新增模块）。
    *   [ ] **第四章 软件实施与实验运行 (全体成员)** - 详细记录软件实现过程、数据库测试、详尽的实验结果与深入分析（图表结合，重点分析遮挡、干扰物、高频特征利用的效果）。
...
*   **任务13：准备答辩PPT (分工合作，组长整合)**
    *   [ ] 内容应包括：项目背景、研究目标、方案设计（突出创新点如多样化遮挡、干扰物检测、高频特征利用）、算法实现、实验结果与深入分析（量化各项改进的效果）、结论与展望、个人贡献。
    *   [ ] PPT制作美观大方，逻辑清晰。
*   **任务14：准备成果演示 (负责编码和测试的成员)**
    *   [ ] 准备好可运行的检测系统和具有代表性的原始、多样化遮挡及干扰物测试样例。
    *   [ ] 确保演示流畅，能清晰展示系统功能、对遮挡和干扰的处理能力以及高频特征利用的效果。
*   **任务15：答辩演练 (全体成员)**
    *   [ ] 模拟答辩场景，互相提问，熟悉答辩流程。
    *   [ ] 针对可能被问到的技术细节（特别是新增部分）、实验结果、项目难点等进行准备。
*   **任务16：参加答辩 (全体成员)**
...
*   **团队成员各自阅读老师提供的三篇参考文献。**
*   **理解并确认项目技术路线。** (详细规划请参考：[项目技术路线概要](./technical_path_summary_updated.md))
